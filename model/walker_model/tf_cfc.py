...
class CfcCell(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        for i in range(self.hparams["backbone_layers"]):
            ...
            self.backbone.append(tf.keras.layers.Activation(backbone_activation, name=f"Activation_Backbone_{i}"))
            self.backbone.append(tf.keras.layers.Dropout(self.hparams["backbone_dr"], name=f"Dropout_Backbone_{i}"))
        ...
        if self._minimal:
            ...
            self.ff1 = tf.keras.layers.Dense(
                self.units,
                None,  # activation='relu',  # use tanh instead of lecun_tanh
                kernel_regularizer=tf.keras.regularizers.L2(
                    self.hparams["weight_decay"]
                ),
                name="ff1"
            )
            self.ff1_activation = tf.keras.layers.Activation(tf.tanh, name="ff1_activation")

            self.ff2 = tf.keras.layers.Dense(
                self.units,
                None,  # tf.tanh, #activation='relu',  # use tanh instead of lecun_tanh
                kernel_regularizer=tf.keras.regularizers.L2(
                    self.hparams["weight_decay"]
                ),
                name="ff2"
            )
            self.ff2_activation = tf.keras.layers.Activation(tf.tanh, name="ff2_activation")
    ...
    def get_validation_data(self, inputs, states_out):
        ...
        for layer in self.backbone.layers:
            ...
            if isinstance(layer, tf.keras.layers.Dense):
                ...
                x = outputs
                result[f"backbone_{layer.name}"] = {"in": backbone_ins, "noActivation_out": backbone_outs}
            if isinstance(layer, tf.keras.layers.Activation):
                outputs = layer(x)
                backbone_ins = x.numpy()
                backbone_outs = outputs.numpy()
                x = outputs
                result[f"backbone_{layer.name}"] = {"in": backbone_ins, "activation_out": backbone_outs}
        ...
        result[f"ff1_ff1"] = {"in": x.numpy(), "noActivation_out": ff1_out.numpy(), "activation_out": ff1_activation_out}
        result[f"ff2_ff2"] = {"in": x.numpy(), "noActivation_out": ff2_out.numpy(), "activation_out": ff2_activation_out}
        result[f"ta_ta"] = {"in": x.numpy(), "noActivation_out": t_a_out.numpy()}
        result[f"tb_tb"] = {"in": x.numpy(), "noActivation_out": t_b_out.numpy()}
